...
{
    public class MySqlRestaurantMenuDAO:MySqlGatewayConnection
    {
        ...
        public PackageCategoryButton GetPackagePackageCategory(PackageItemButton itemButton)
        {
            ...
            DataSet DS = new DataSet();
            DataTable DT = new DataTable();
            Query = String.Format("SELECT * FROM rcs_package_category where category_id like '%{0}%'  AND package_id={1} AND option_name='{2}'", itemButton.CategoryId, itemButton.PackageCategoryButton.PackageId, itemButton.OptionName);
        ...
        {
            List<int> starterIds = new List<int>(0);
            ...
            while (Reader.Read())
            {
                try
                {
                    starterIds.Add(Convert.ToInt32(Reader["id"]));
                }
                catch (Exception exception)
                {
                    ErrorReportBLL aErrorReportBll = new ErrorReportBLL();
        ...
        public List<ReceipeCategoryButton> GetAllCategory()
        {
            ...
            foreach (DataRow dataRow in DT.Rows)
            {
                ...
                aReceipeCategoryButton.MaxRow = Convert.ToInt32(dataRow["max_row"]);
                aReceipeCategoryButton.Color = Convert.ToString(dataRow["color"]);
                aReceipeCategoryButton.CategoryWidth = Convert.ToInt32(dataRow["width"]) == 0 ? 12 : Convert.ToInt32(dataRow["width"]);
                aReceipeCategoryButton.CategoryHeight = Convert.ToInt32(dataRow["height"]);
                aReceipeCategoryButton.CategoryName = Convert.ToString(dataRow["name"]);
                aReceipeCategoryButton.Description = Convert.ToString(dataRow["description"]);
                aReceipeCategoryButton.HasSubcategory = Convert.ToInt32(dataRow["has_subcategory"]);
        ...
        public bool GetReceipeOptionsByItemId(int recipeMenuItemId)
        {
            ...
            Query = String.Format("select *  FROM   rcs_recipe_option as ro INNER JOIN " +
                                  "rcs_restaurant_recipe_option as rro ON ro.id = rro.option_id  where rro.recipe_id= {0} and ro.is_offline=1 order by ro.title ;",
                   recipeMenuItemId);
        ...
        internal double GetPrice(int categoryId, int subcategoryId, int packageId, int  recipeId=0)
        {
            ...
            if (recipeId > 0)
            {
                ...
                bool readConnection1 = CommonMethodConectionReaderClose.Connection_ReaderClose(Connection, Reader);
            }
            ...
            if (price == 0 && categoryId >0 && subcategoryId >0 )
            {
                ...
                bool readConnection2 = CommonMethodConectionReaderClose.Connection_ReaderClose(Connection, Reader);
                if (RecipeId > 0)
                {
        ...
        internal int GetRecipeIdByCatAndSubcat(int categoryId, int subcategoryId)
        {
                ...
                while (Reader.Read())
                {
                    ...
                    try
                    {
                        recipe = Convert.ToInt32(Reader["id"]);
        ...
        internal int GetTypeStatusByID(int Id)
        {
            ...
            DataSet DS = new DataSet();
            DataTable DT = new DataTable();
            Query = String.Format("SELECT has_underline FROM `rcs_recipe_type` WHERE id ={0};",Id);
            ...
            while (Reader.Read())
            {
                ...
                try
                {
                    has_underline = Convert.ToInt32(Reader["has_underline"]);
        ...
        internal string GetRecipeNameById(int Id)
        {
            ...
            Query = String.Format("SELECT receipt_name FROM  `rcs_recipe`  WHERE id ={0};", Id);
            ...
            while (Reader.Read())
            {
                ...
                try
                {
                    recipeName = Convert.ToString(Reader["receipt_name"]);
