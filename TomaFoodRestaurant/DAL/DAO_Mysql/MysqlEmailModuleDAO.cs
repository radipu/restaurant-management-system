...
{
    public class MysqlEmailModuleDAO : MySqlGatewayConnection
    {
        ...
        public EmailModule GetEmailModule()
        {
            ...
            Query = String.Format("SELECT em.*, re.restaurant_id, re.status as isActive FROM rcs_restaurant_email re" +
                " JOIN rcs_email_modules em ON em.id=re.email_module_id where re.status='{0}' AND em.status='{0}' LIMIT 1;", "active");
