...
{
    public class MySqlPackageDAO : MySqlGatewayConnection
    {
        ...
        public List<ReceipeCategoryButton> GetCategoryList(List<int> catogries)
        {
            ...
            for (int i = 0; i < catogries.Count(); i++)
            {
                ...
                Query = String.Format("SELECT rcs_recipe_category.id,rcs_recipe_category.parent_category_id,rcs_recipe_category.recipe_type,rcs_recipe_category.restaurant_id,rcs_recipe_category.sort_order,rcs_recipe_category.max_row,rcs_recipe_category.color,rcs_recipe_category.width,rcs_recipe_category.height,rcs_recipe_category.description,case when(rcs_recipe_category.name='') then rcs_recipe_categories.has_subcategory else rcs_recipe_category.has_subcategory end As has_subcategory,case when(rcs_recipe_category.name='') then rcs_recipe_categories.name else rcs_recipe_category.name end AS name  FROM rcs_recipe_category   left join  rcs_recipe_categories on rcs_recipe_categories.id= rcs_recipe_category.parent_category_id  where rcs_recipe_category.id={0};", catogries[i]);
        ...
        public List<PackageItemButton> GerSubCategoryList(List<int> subCategories, PackageCategoryButton category)
        {
            ...
            {
                Query = String.Format("SELECT rcs_package_recipe.option_name,rcs_package_category.items,rcs_package_recipe.add_price, rcs_recipe.name, rcs_recipe.subcategory_id,rcs_recipe.category_id," +
                                                                "rcs_recipe.receipt_name,rcs_recipe_subcategory.sort_order,rcs_recipe_subcategory.color,rcs_recipe_subcategory.width,rcs_recipe_subcategory.height, rcs_package_recipe.package_id,rcs_package_recipe.recipe_id, rcs_recipe_subcategory.title" +
                                                                 ...
                                                                 "  LEFT JOIN rcs_package_category ON rcs_package_category.package_id =  rcs_package_recipe.package_id AND rcs_package_category.option_name = rcs_package_recipe.option_name " +
                ...
                if (DT.Rows.Count==0)
                {
                    aList=aList.Concat(GetDisplayPackageRecipeItemWhereIncludenotinpackage(subCategories[i], category)).ToList();
                }
                ...
                foreach (DataRow dataRow in DT.Rows)
                {
                    ...
                    aPackageItemButton.CountPackageItem = Convert.ToInt32(dataRow["items"]);
                    aPackageItemButton.FreeOptionMds = category.FreeOptionMds;
                    aPackageItemButton.Text = dataRow["title"].ToString();
                    aPackageItemButton.Height = 40;
                    // Convert.ToInt32(dataRow["height"]);
        ...
        public List<PackageItemButton> GetDisplayPackageRecipeItemWhereIncludenotinpackage(int subCategories, PackageCategoryButton category)
        {
            DataSet DS = new DataSet();
            DataTable DT = new DataTable();
            ...
            RestaurantMenuBLL aRestaurantMenuBll = new RestaurantMenuBLL();
            List<PackageItemButton> aList = new List<PackageItemButton>();
            Query = String.Format("SELECT IFNULL(rcs_package_recipe.option_name,'Empty') as option_name,IFNULL(rcs_package_recipe.add_price,0.0)as add_price, rcs_recipe.name, rcs_recipe.subcategory_id,rcs_recipe.category_id," +
                                                               "rcs_recipe.receipt_name,rcs_recipe_subcategory.sort_order,rcs_recipe_subcategory.color,rcs_recipe_subcategory.width,rcs_recipe_subcategory.height, IFNULL(rcs_package_recipe.package_id,0) as package_id,IFNULL(rcs_package_recipe.recipe_id,0) as recipe_id , rcs_recipe_subcategory.title" +
                                                                " FROM rcs_package_recipe right JOIN  rcs_recipe ON rcs_package_recipe.recipe_id = rcs_recipe.id Inner Join rcs_recipe_subcategory on  rcs_recipe.subcategory_id = rcs_recipe_subcategory.id" +
                                                               " where   rcs_recipe.subcategory_id={0}  Group By rcs_recipe.subcategory_id ;",
                                                                subCategories);
        ...
        public DataTable GetPackagerReceipeByPackageItemWithSubCat(PackageCategoryButton catid)
        {
            ...
            try
            {
                ...
                if (catid.all_recipe > 0)
                {
                  //  Query = String.Format("SELECT rcs_recipe.* FROM rcs_recipe WHERE rcs_recipe.subcategory_id IN (@subcategory_id) AND rcs_recipe.category_id IN (@category_id);");
                    ...
                    Query = String.Format(
                                   ...
                                   "rcs_recipe.id where rcs_package_recipe.option_name=@option_name AND rcs_package_recipe.package_id=@package_id AND rcs_recipe.subcategory_id IN (@subcategory_id) AND rcs_recipe.category_id IN (@category_id) order by rcs_recipe.name ASC , rcs_recipe.sort_order asc;");
                ...
                command.Parameters.AddWithValue("@category_id", catid.CategoryId);
